...
**Not ready to submit yet.** 4 blocking issues + 3 listing items must be addressed first. Otherwise the app's foundations are sound: GDPR webhooks, CSP headers, OAuth state HMAC, sanitize-html, AES-256-GCM token encryption, embedded App Bridge wrapper, read-only scopes, multi-tenant isolation, and PII hashing in logs are all in place.

| Tier | Count |
|------|-------|
| BLOCKERS (fix before submit) | 4 |
| HIGH (strongly recommended) | 5 |
| MEDIUM (defensive) | 4 |
| LISTING-ONLY (no code change) | 5 |
| Verified good | 16 |
...
### B1 — Production `shopify.app.toml` uses deprecated GDPR webhook format
...
### B2 — `<s-app-nav>` is missing the `name=` attribute
...
### B4 — App lands on `/app/inbox` before the merchant has connected an inbox
...
### H1 — `postMessage('*')` from email iframe — receiver should validate `event.origin`
...
### H2 — Iframe sandbox keeps `allow-popups` unnecessarily
...
### H3 — No rate limiting on `/api/reply-draft` (LLM-cost DoS)
...
### H4 — No CSRF token on draft mutations (relies entirely on Shopify session JWT)
...
### H5 — `app.tsx` builds a synthetic `host` parameter from `shop` query
...
### M1 — `app.support.tsx` is a feature page, not a support-contact page
...
### M2 — Bulk DB-mutation actions (resync, redraft, reanalyze) don't have undo / confirm
...
### M3 — Public OAuth callback (`/mail-auth`) lacks rate limiting
...
### M4 — `app.inbox.tsx` second `findMany` (analyzed-per-thread) has no `take`
